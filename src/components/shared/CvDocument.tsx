...
export function CvDocument({ profile: m }: { profile: StudentProfile }) {
  return (
    <div>
      <div className="font-serif text-[28px] font-semibold tracking-[-.3px] text-navy">{m.name}</div>
      <div className="mt-0.5 text-[13px] font-semibold text-gold">{m.headline}</div>
      <div className="mt-2 flex flex-wrap gap-x-3.5 gap-y-1.5 text-[11px] text-[#444]">
        <span>{m.email}</span><span>{m.phone}</span><span>{m.city}, India</span>
        {m.links.map((l) => <span key={l.label}>{l.url}</span>)}
      </div>
      <div className="mt-3 h-0.5 bg-navy" />

      <div className="mt-[10px]">
        <CvSectionTitle>Summary</CvSectionTitle>
        <p className="text-[12px] leading-[1.4] text-[#333]">{m.summary}</p>
      </div>
      ...
      <div className="mt-[10px]">
        ...
        {m.internships.map((x) => (
          <div key={x.role + x.co} className="mb-1.5">
      ...
      <div className="mt-[10px]">
        ...
        {m.projects.map((p) => (
          <div key={p.name} className="mb-1.5">
            ...
            <p className="text-[12px] leading-[1.4] text-[#333]">{p.desc}</p>
      ...
      <div className="mt-[10px] grid grid-cols-2 gap-x-[26px]">
        <div>
      ...
      <div className="mt-[10px]">
        ...
        {m.positions.map((p) => (
          <div key={p.role} className="mb-1.5">
            ...
            <p className="text-[12px] leading-[1.4] text-[#333]">{p.d}</p>
          </div>
        ))}
      </div>
    </div>
